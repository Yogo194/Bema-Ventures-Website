...
export default function EssayAuthor() {
  return (
    <section className="essay-author-section">
      <div className="essay-author-inner">
        <div className="essay-author-card">
          <div className="essay-author-photo">
            <img src={`/assets/${author.photo}`} alt={author.name} />
          </div>
          <div className="essay-author-content">
            <div className="section-label">About the author</div>
            <h2>{author.name}</h2>
            <p className="essay-author-role">{author.role}, Bema Ventures &amp; Services Limited</p>
            <p>{author.bio}</p>
            <Link to="/about" className="btn btn-text">
              More about our leadership
              <svg width="18" height="15" viewBox="0 0 18.0006 15.0008" fill="none" aria-hidden="true">
